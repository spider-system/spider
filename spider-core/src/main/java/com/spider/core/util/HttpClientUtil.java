...
public class HttpClientUtil {
	...
	private static void init() {
        try {
            ...
            SocketConfig socketConfig = SocketConfig.custom().setSoTimeout(GlobConts.TIMEOUT).setTcpNoDelay(true).build();
            ...
            requestConfig = RequestConfig.custom().setSocketTimeout(GlobConts.TIMEOUT).
	...
	public static CloseableHttpResponse getResponse(HttpRequestBase request) throws IOException {
		...
		request.setHeader("User-Agent", GlobConts.userAgentArray[new Random().nextInt(GlobConts.userAgentArray.length)]);
...
//		}
	...
	public static org.apache.http.client.config.RequestConfig.Builder getRequestConfigBuilder(){
		return RequestConfig.custom().setSocketTimeout(GlobConts.TIMEOUT).
				setConnectTimeout(GlobConts.TIMEOUT).
				setConnectionRequestTimeout(GlobConts.TIMEOUT).
				setCookieSpec(CookieSpecs.STANDARD);
	}
